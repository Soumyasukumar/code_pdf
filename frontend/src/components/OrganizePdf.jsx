  ...
  return (
    ...
    >
      <div
        ...
        ${isDragging ? 'border-red-500 shadow-xl scale-105 z-50' : 'border-transparent hover:border-red-300'}
        `}
      >
        <div className="relative overflow-hidden w-32 h-44 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 flex items-center justify-center">
          {page.src ? (
            <img
              src={page.src}
              alt={`Page ${index + 1}`}
            ...
            <span className="text-gray-400 dark:text-gray-500 font-bold text-xl">{index + 1}</span>
          )}
      ...
      <div className="mt-2 text-sm font-medium text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 px-3 py-0.5 rounded-full border border-gray-200 dark:border-gray-600 shadow-sm">
        {index + 1}
      </div>
    </div>
  );
...
function OrganizePdf() {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      setPages(
        response.data.thumbnails.map((thumb, i) => ({
          ...thumb,
          id: `page-${i}`,
          originalIndex: i,
          rotation: 0,
  ...
  const handleOrganize = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/organize-pdf', formData, {
        responseType: 'blob',
      });
    ...
    <>
      ...
      <section className="py-16 px-4 bg-gradient-to-b from-red-50 to-white dark:from-gray-800 dark:to-gray-900">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Drag to reorder, rotate, or delete pages. Download your custom PDF.
          </p>
        </div>
      </section>
            ...
            {!file ? (
              <div className="p-12 text-center">
                <label className="flex flex-col items-center justify-center w-full max-w-2xl mx-auto h-64 border-2 border-dashed rounded-2xl cursor-pointer bg-white dark:bg-gray-700 hover:bg-red-50 dark:hover:bg-red-900/10 transition-all shadow-sm hover:shadow-md group border-red-200 dark:border-red-700">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">or drop PDF here</p>
                  </div>
                  <input type="file" className="hidden" accept="application/pdf" onChange={handleFileChange} />
                </label>
                {loading && (
                  <div className="mt-8 flex items-center justify-center text-red-600 dark:text-red-400 font-medium animate-pulse">
                    <span className="text-2xl mr-2">Generating Previews...</span>
                  </div>
                )}
                {error && <div className="mt-8 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-4 py-2 rounded-lg">{error}</div>}
              </div>
            ) : (
              <div className="flex flex-col lg:flex-row">
                ...
                <div className="w-full lg:w-96 bg-gray-50 dark:bg-gray-700 p-6 space-y-6">
                  ...
                  <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-4 border border-red-100 dark:border-red-700">
                  ...
                  <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400 border-b pb-2">
      ...
      <section className="py-16 px-4 bg-gray-50 dark:bg-gray-900">
        <div className="max-w-6xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-10">Why Organize PDFs with Us?</h2>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow">
              <div className="text-4xl mb-3">Drag</div>
              <h3 className="font-bold text-lg mb-1">Drag & Drop</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Reorder pages with touch or mouse.
              </p>
            </div>
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow">
              <div className="text-4xl mb-3">Rotate</div>
              <h3 className="font-bold text-lg mb-1">Rotate & Delete</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Fix orientation and remove unwanted pages.
              </p>
            </div>
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow">
